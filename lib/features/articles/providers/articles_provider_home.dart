...
class ArticlesNotifier extends StateNotifier<AsyncValue<ArticlesList>> {
  ...
  Future<void> loadArticles({
    int page = 1,
    int limit = 50, // Load 50 articles initially to show more content
    String? category,
  ...
  Future<void> loadMore() async {
    ...
    // Load next batch of articles - use smaller limit for "load more"
    await loadArticles(page: currentState.page + 1, limit: 20);
  }
  
  Future<void> refresh() async {
    // Reset to load articles with high limit to get all available
    await loadArticles(page: 1, limit: 100);
  }
  ...
  Future<void> loadArticlesByCategory(String category) async {
    await loadArticles(
      ...
      sortBy: 'publishedAt',
      order: 'desc',
      limit: 30, // Load reasonable amount per category
    );
  }
}
